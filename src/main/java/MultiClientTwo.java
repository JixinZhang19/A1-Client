...
//    private static final int THREADS_NUM_SECOND = 168;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        int postReqEachThreadSecond = Integer.parseInt(args[0]);
        int threadsNumSecond = Integer.parseInt(args[1]);

        ExecutorService executor = Executors.newCachedThreadPool();
        BlockingQueue<SkierTask> queue = new LinkedBlockingQueue<>();
        ...
        for (int i = 0; i < threadsNumSecond; i++) {
            executor.execute(new SkierConsumer(queue, startLatch, successCount, failCount, postReqEachThreadSecond));
        }
